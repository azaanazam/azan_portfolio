...
var typed = new Typed('#profession', {
    strings: ['Web Developer','Software Developer','Full Stac Developer'],
    typeSpeed: 150,
    backSpeed:150,
    loop:true
  });
  ...
  let html=setInterval(()=>{
    ...
    htmlprog.style.background=`conic-gradient(rgb(35, 201, 121) ${3.6*h}deg ,rgb(8, 10, 11) 00deg)`
    if(h==90){
        clearInterval(html)
    }
  },30)
  ...
  let css=setInterval(()=>{
    ...
    cssprog.style.background=`conic-gradient(rgb(35, 201, 121) ${3.6*c}deg ,rgb(8, 10, 11) 00deg)`
    if(c==80){
        clearInterval(css)
    }
  },30)
  ...
  let js=setInterval(()=>{
    j++;
    jsvalue.innerHTML=`${h}%`
    jsprog.style.background=`conic-gradient(rgb(35, 201, 121) ${3.6*j}deg ,rgb(8, 10, 11) 00deg)`
    if(h==75){
        clearInterval(js)
    }
  },30)
